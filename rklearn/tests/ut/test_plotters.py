...
"""
Usage:
(...) $ cd <project_top_dir>/rklearn/tests
(...) $ python ut/test_plotters.py --conf=config/config.yaml
"""
...
class TestPlotters(unittest.TestCase):
    ...
    def setUp(self):
        ...
        self.X, self.y = load_iris_binary_data(csv_file=self.csv_file,
                                               features=self.features, pos_class=self.pos_class,
                                               neg_class=self.neg_class, logger=logger)
        assert (self.X is not None)
        assert (self.y is not None)
        end_prep = time.time()
        logger.info("- Iris dataset loaded in {} seconds".format(end_prep - start_prep))
        logger.info("- self.X.shape = {}, self.y.shape = {}".format(self.X.shape, self.y.shape))

    ##########################################
    # test_plot_learning_curves_cv_scores()  #
    ##########################################

    def test_plot_learning_curves_cv_scores(self):
        assert (logger is not None)

        logger.info("test: test_plot_learning_curves_cv_scores()")
    ...
    def test_plot_learning_curves_scores_2(self):
        assert (logger is not None)
        logger.info("test: test_plot_learning_curves_scores_2()")
        logger.info("display: Should draw learning curves + fit times vs data.")
    ...
    def test_plot_learning_curves_scores_3(self):
        ...
        plot_learning_curves_cv_scores(self.perceptron,
                                       ...
                                       title="Learning curves for Perceptron - epochs = {} - lr = {}".format(
                                           self.n_epochs, self.lr),
                                       logger=logger).savefig(self.learning_curves_fig, dpi=300)
