###################################################
## test_rklearn_adaline_binary_iris_training.py  ##
###################################################

"""
Test the Adaline classifiers on the iris dataset:
    - AdalineGD: batch GD
    - AdalineSGD: stochastic GD
Since thePerceptron is a binary classifier, then only 2 categories of flowers are selected: Setosa, and Versicolor.

Usage:
$ cd <top_dir>
$ python rklearn/tests/test_rklearn_adaline_binary_iris_training.py --conf rklearn/tests/config/config.yaml
"""
...
from rklearn.adaline import AdalineGD
from rklearn.tests.data_loaders import load_iris_binary_data
...
def main(
    ...
    train_cost_fig = "/tmp/iris-train-error-fig.png"):
    ...
    try:
        ...
        logger.info("===========================")
        logger.info("Fitting an AdalineGD")
        logger.info("===========================")
        ...
        logger.info("Training costs (latest {} values) for all epochs = ...{}".format(nb_costs, ada.cost_[-nb_costs:]))

        suffix = "adalineGD-lr-{}-epochs-{}".format(lr, n_epochs)
        fig = train_cost_fig.format(suffix)
        plot_simple_sequence(ada.cost_,
                xlabel="Epochs", ylabel="Cost",
                title="Training costs = f(Epochs) - lr = {}".format(lr)).savefig(fig, dpi=300)
...
if __name__ == '__main__':
    ...
    logger = init_logger(name="test_rklearn_adaline",
            level=logging.getLevelName(config["logger"]["log_level"].upper()))

    logger.info("")
    logger.info("##################################################")
    logger.info("## test_rklearn_adaline_binary_iris_training.py ##")
    logger.info("##################################################")
    logger.info("")
