##############################################################
## test_rklearn_sklearn_perceptron_binary_iris_training.py  ##
##############################################################

"""
Test the Scikit Learn Perceptron classifier on the iris dataset.
Since the Perceptron is a binary classifier, then only 2 categories of flowers are selected: Setosa, and Versicolor.

Usage:
$ cd <top_dir>
$ python rklearn/tests/test_rklearn_sklearn_perceptron_binary_iris_training.py --conf=rklearn/tests/config/config.yaml
"""
...
def main(
    ...
    try:
        ...
        logger.info("==============================")
        logger.info("Fitting a sklearn perceptron")
        logger.info("==============================")

        logger.info("Hyperparameters:")
        logger.info("- lri (e.g. eta0) = {}".format(lr))

        start_fit = time.time()
        ppn = Perceptron(eta0=lr, random_state=1)   # The Scikit Learn Perceptron
        ...
        logger.info("Self Prediction Accuracy (e.g. ppn.score() method): {:.3f}".format(ppn.score(X_test, y_test)))
